...
def place_reviews(place_id):
    ...
    if not obj_place:
        abort(404)
    return jsonify([review.to_dict() for review in obj_place.reviews])
...
def post_review(place_id):
    ...
    obj_review = Review(**new_review)
    setattr(obj_review, 'place_id', place_id)
    storage.new(obj_review)
    storage.save()
    return make_response(jsonify(obj_review.to_dict()), 201)
